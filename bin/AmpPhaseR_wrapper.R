...
if (result$success) {
  result$read_phase %>% 
    ...
    mutate(freq = n / sum(n)) %>% 
    arrange(desc(n)) %>% 
    write_tsv(str_c(opts$out_prefix, '_read_state_freq.tsv.gz'))
  ...
  result$phase_summary %>%
    mutate(phase_copy_num = as.integer(copy_num) * ratio / sum(ratio)) %>% 
    select(phase, count, freq, phase_copy_num) %>%
